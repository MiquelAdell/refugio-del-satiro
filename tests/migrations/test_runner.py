...
class TestMigrationRunner:
    ...
    def test_applies_catalog_metadata_migration(self) -> None:
        conn = get_memory_connection()
        applied = run_migrations(conn)
        assert "009_add_catalog_metadata" in applied
        conn.close()

    def test_catalog_metadata_migration_preserves_existing_rows(self) -> None:
        conn = get_memory_connection()
        migrations = sorted(MIGRATIONS_DIR.glob("*.sql"))
        schema_eight_migrations = [
            migration for migration in migrations if migration.stem < "009"
        ]
        for migration in schema_eight_migrations:
        ...
        conn.executemany(
            ...
            ((migration.stem,) for migration in schema_eight_migrations),
        )
        conn.execute(
            "INSERT INTO games "
            "(bgg_id, name, thumbnail_url, year_published) "
            "VALUES (99, 'Existing game', 'https://t.jpg', 2020)"
        )
        conn.commit()

        applied = run_migrations(conn)

        assert applied == ["009_add_catalog_metadata"]
