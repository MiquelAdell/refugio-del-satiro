...
Env vars:
    TEST_MEMBER_EMAIL    (default: e2e-member@example.invalid)
    TEST_MEMBER_PASSWORD (default: e2e-test-password-member)
    TEST_ADMIN_EMAIL     (default: e2e-admin@example.invalid)
...
def _accounts_from_env() -> list[TestAccount]:
    return [
        TestAccount(
            label="TEST_MEMBER",
            email=os.environ.get("TEST_MEMBER_EMAIL", "e2e-member@example.invalid"),
        ...
        TestAccount(
            label="TEST_ADMIN",
            email=os.environ.get("TEST_ADMIN_EMAIL", "e2e-admin@example.invalid"),
