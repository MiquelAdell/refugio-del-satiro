...
- [x] C.4 [FE] Keep `prestamos_session` localStorage key; tests green.

## D. RPG catalog UI [FE]

- [x] D.1 [FE] `types/rpg.ts` (`RpgItem`, `RpgQuery`, `applyRpgQuery`),
      `hooks/useRpgItems.ts`.
- [x] D.2 [FE] `CatalogTypeToggle` (legacy pill look) + `PoweredByBgg`
      (svg from legacy repo) on both catalog pages.
- [x] D.3 [FE] `RpgCard`, `RpgCatalogPage` (search + name/rating sort only),
      `RpgDetailPage` (no borrow UI, RPGGeek link); routes `/juegos-de-rol`,
      `/rol/:slug`.
- [x] D.4 [FE] Vitest: search/sort with concrete orders; CatalogPage toggle +
      banner without filter regressions.

## E. E2E + docs

- [x] E.1 Update redirect/guest/site-shell/visual specs for `/ludoteca`;
      add `/prestamos?query` redirect cases.
- [x] E.2 Guest RPG journey: toggle → `/juegos-de-rol` → `/rol/:slug` →
      no borrow UI, banner visible. Seed one rpgitem.
- [x] E.3 README + scraper/README path updates.
